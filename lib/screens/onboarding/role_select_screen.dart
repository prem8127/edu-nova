...
class _RoleSelectScreenState extends ConsumerState<RoleSelectScreen> {
  ...
  /// Google sign-in. There's no backend yet, so this signs the person
  /// straight into the platform using the same seeded demo accounts the
  /// email flow uses — a real OAuth client id can be dropped in here later
  /// (via the `google_sign_in` package) without touching the UI above.
  Future<void> _continueWithGoogle() async {
    if (_tab == _Tab.student) {
      // A brand-new Google account still needs a class/age, same as email
      // sign-up — hand off to onboarding to collect that.
      context.go(AppRoutes.onboarding);
      return;
    }
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            hintText: switch (_tab) {
                              _Tab.student => 'you@email.com',
                              _Tab.teacher => 'teacher@edunova.demo',
                              _Tab.admin => 'admin@edunova.demo',
                            },
                        ...
                        GradientButton(
                          label: _tab == _Tab.student
                              ? (_isSignUp ? 'Create account' : "I'm a Student — Get Started")
                              : (_isSignUp ? 'Create account' : 'Sign in'),
                          icon: _tab == _Tab.student && !_isSignUp
                        ...
                        _GoogleButton(
                          ...
                          onTap: _googleSubmitting ? null : _continueWithGoogle,
                        ),
                        const SizedBox(height: 14),
                        Row(
                        ...
                        const Text(
                          'By continuing you agree to Aditya Globals\'s Terms of Service and Privacy Policy.',
                          textAlign: TextAlign.center,
...
/// Full-width "Continue with Google" pill: white surface, subtle border,
/// four-colour "G" mark, and a built-in loading state so it reads as a
/// real, working sign-in option rather than a placeholder icon.
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: const BorderSide(color: AppBrand.line, width: 1.2),
      ),
      child: InkWell(
        borderRadius: BorderRadius.circular(AppBrand.radiusPill),
        onTap: onTap,
...
/// Minimal four-colour Google "G" mark, drawn with stock Material icons so
/// no external asset/SVG is required.
...
class _GoogleGPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    arc(-40, -130, const Color(0xFF4285F4)); // blue (right)
    arc(-170, -80, const Color(0xFF34A853)); // green (bottom-left)
    arc(-250, -50, const Color(0xFFFBBC05)); // yellow (left)
    arc(-300, -60, const Color(0xFFEA4335)); // red (top)

    // Horizontal bar of the "G".
    final barPaint = Paint()..color = const Color(0xFF4285F4);
    canvas.drawRect(
      Rect.fromLTWH(center.dx, center.dy - stroke / 2, r * .95, stroke),
      barPaint,
    );
  }
