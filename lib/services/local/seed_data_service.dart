...
class SeedDataService {
  ...
  static Future<void> seedPlatformContent({
    ...
    //
    // NOTE (mid-migration): userRepo is now SupabaseUserRepository, whose
    // upsertUser() writes to `profiles`, where `id` is a uuid FK into
    // auth.users. These SeedIds (e.g. "seed_teacher_tech") are neither
    // valid uuids nor real auth.users rows, so this write always fails
    // there (22P02 / FK violation) — it only ever worked against the old
    // local mock repo. Course/quiz/schedule repos are still local and
    // just store these ids as plain strings, so they don't need a real
    // profile row to function. Swallow the failure here so demo course
    // content still seeds; once teachers are real Supabase Auth accounts
    // (later migration step), replace this whole block instead of
    // catching around it.
    try {
      ...
      // Expected against the real Supabase backend until seed teachers/
      // admin exist as actual auth.users rows — see note above.
    }
